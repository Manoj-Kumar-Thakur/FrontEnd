...
class CustomerUpdateForm extends React.Component {
  ...
  displayValidationErrors = (fieldName) => {
    ...
    if (validator && !validator.valid) {
      const errors = validator.errors.map((info, index) => {
        return (
          <span style={errorStyle} key={index}>
            * {info}
          </span>
        );
      });
  ...
  onDoorNoChange = (e) => {
    this.setState((state) => ({
      address: { ...state.address, doorNo: e.target.value },
    }));
  };
  ...
  onAreaChange = (e) => {
    this.setState((state) => ({
      address: { ...state.address, area: e.target.value },
    }));
  };
        ...
        <form onSubmit={this.onSubmit}>
          ...
          <FormControl fullWidth>
            <TextField
              ...
              onChange={(event) =>
                this.handleInputChangeCustomer(event, "name")
              }
            />
          </FormControl>
          ...
          <FormControl fullWidth>
            <TextField
              ...
              onChange={(event) =>
                this.handleInputChangeCustomer(event, "contactNo")
              }
            />
          </FormControl>
          ...
          <FormControl fullWidth>
            <TextField
              ...
              onChange={(event) => this.handleInputChangeCustomer(event, "dob")}
              InputLabelProps={{
                shrink: true,
              }}
            />
          ...
          <FormControl fullWidth>
            <TextField
              ...
              id="standard-textarea"
              label="Door Number"
              value={this.state.address.doorNo}
              onChange={this.onDoorNoChange}
            />
          </FormControl>
          <br />
          ...
          <FormControl fullWidth>
            <TextField
              ...
              onChange={(event) => this.handleInputChange(event, "pinCode")}
            />
          </FormControl>
          ...
          {this.state.error && (
            <b className="m-1 text-danger">{this.state.error}</b>
          )}
