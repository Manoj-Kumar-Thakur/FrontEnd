...
const Product = () => {
  ...
  const loadProduct = async () => {
    const result = await axios
      .get(`http://localhost:9191/api/oss/getProduct/${productId}`)
      .catch((err) => {
  ...
  const deleteProduct = async (productId) => {
    await axios
      .delete(`http://localhost:9191/api/oss/deleteProduct/${productId}`)
      .catch((err) => {
        console.log("Error", err);
      });
  ...
  return (
    <div>
      ...
      <ul class="list-group-item">
        ...
        <li class="list-group-item list-group-item-info">
          {" "}
          <h3>Product Color : {product.color}</h3>{" "}
        </li>
        <li class="list-group-item list-group-item-info">
          {" "}
          <h3>Product Size : {product.size}</h3>{" "}
        </li>
        <li class="list-group-item list-group-item-info">
          {" "}
          <h3>Product Mrp : {product.mrp}</h3>{" "}
        </li>
        <li class="list-group-item list-group-item-info">
          {" "}
        ...
        <li class="list-group-item list-group-item-info">
          {" "}
          <h3>Product Estimated Delivery : {product.estimatedDelivery}</h3>{" "}
        </li>
      </ul>
      <Grid container spacing={3}>
        <Grid item xs={3}>
          <Link to={`/order`}>
            <Button style={style}>Order </Button>
          </Link>
        </Grid>
        <Grid item xs={3}>
          <Link to={`/product`}>
            <Button style={style}>See more Products </Button>
          </Link>
        </Grid>
      </Grid>
    </div>
